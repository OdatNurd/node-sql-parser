...
describe('create', () => {
  ...
  describe('create table with basic', () => {
    it('should support create table', () => {
      ...
      expect(getParsedSql(`create table dbname.tableName (id INT PRIMARY KEY NOT NULL AUTO_INCREMENT, date DATETIME not null unique key);`))
        .to.equal('CREATE TABLE "dbname"."tableName" ("id" INT NOT NULL AUTOINCREMENT PRIMARY KEY, "date" DATETIME NOT NULL UNIQUE KEY)');
      ...
      expect(getParsedSql('CREATE TABLE `Person` (`id_Person` int(10) unsigned NOT NULL AUTO_INCREMENT, `id_person_gender` int(11) unsigned zerofill NOT NULL, `id_person_origin` int(11) zerofill NOT NULL, `age` int(11) NOT NULL, `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP, `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, updateTime datetime(0) NULL DEFAULT CURRENT_TIMESTAMP(0) ON UPDATE CURRENT_TIMESTAMP(0) COMMENT "最新更新时间", `is_alive` tinyint(1) DEFAULT NULL, `updated_by` varchar(48) DEFAULT NULL, PRIMARY KEY (`id_Person`), UNIQUE KEY `pft_ge_or` (`id_person_gender`, `id_person_origin`) );'))
        .to.equal('CREATE TABLE "Person" ("id_Person" INT(10) UNSIGNED NOT NULL AUTOINCREMENT, "id_person_gender" INT(11) UNSIGNED ZEROFILL NOT NULL, "id_person_origin" INT(11) ZEROFILL NOT NULL, "age" INT(11) NOT NULL, "created_at" TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP, "updated_at" TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, "updateTime" DATETIME(0) NULL DEFAULT CURRENT_TIMESTAMP(0) ON UPDATE CURRENT_TIMESTAMP(0) COMMENT "最新更新时间", "is_alive" TINYINT(1) DEFAULT NULL, "updated_by" VARCHAR(48) DEFAULT NULL, PRIMARY KEY ("id_Person"), UNIQUE ("id_person_gender", "id_person_origin"))');
      ...
      expect(getParsedSql("CREATE TABLE `action`(`id` int NOT NULL AUTO_INCREMENT, `platform` enum('IOS','ANDROID','PC_WEB','MOBILE_WEB','ETC') NOT NULL DEFAULT 'PC_WEB', `size` ENUM('small', CONCAT('med','ium'), 'large'), `date` datetime NOT NULL,PRIMARY KEY (`id`));"))
        .to.equal('CREATE TABLE "action" ("id" INT NOT NULL AUTOINCREMENT, "platform" ENUM(\'IOS\', \'ANDROID\', \'PC_WEB\', \'MOBILE_WEB\', \'ETC\') NOT NULL DEFAULT \'PC_WEB\', "size" ENUM(\'small\', CONCAT(\'med\', \'ium\'), \'large\'), "date" DATETIME NOT NULL, PRIMARY KEY ("id"))');
    ...
    describe('column definition options', () => {
      it('should support create table with auto_increment', () => {
        expect(getParsedSql(`create temporary table if not exists dbname.tableName (id INT(11) auto_increment primary key) ENGINE = MEMORY`))
          .to.equal('CREATE TEMPORARY TABLE IF NOT EXISTS "dbname"."tableName" ("id" INT(11) AUTOINCREMENT PRIMARY KEY) ENGINE = MEMORY');
      })
      ...
      it('should support create table with engine', () => {
        expect(getParsedSql(`create temporary table if not exists dbname.tableName (id INT(11) auto_increment primary key) ENGINE = innodb`))
          .to.equal('CREATE TEMPORARY TABLE IF NOT EXISTS "dbname"."tableName" ("id" INT(11) AUTOINCREMENT PRIMARY KEY) ENGINE = INNODB');
      })

      it.skip('should support create table with comment', () => {
        expect(getParsedSql(`create temporary table if not exists dbname.tableName (id INT(11) auto_increment primary key comment "id column", name varchar(128) unique key comment "user name") ENGINE = MEMORY`))
          .to.equal('CREATE TEMPORARY TABLE IF NOT EXISTS `dbname`.`tableName` (`id` INT(11) AUTOINCREMENT PRIMARY KEY COMMENT "id column", `name` VARCHAR(128) UNIQUE KEY COMMENT "user name") ENGINE = MEMORY');
      })

      it.skip('should support create table with collate', () => {
        expect(getParsedSql(`create temporary table if not exists dbname.tableName (id INT(11) auto_increment primary key comment "id column" collate utf8_bin, name varchar(128) unique key comment "user name" collate utf8_bin) ENGINE = MEMORY`))
          .to.equal('CREATE TEMPORARY TABLE IF NOT EXISTS `dbname`.`tableName` (`id` INT(11) AUTOINCREMENT PRIMARY KEY COMMENT "id column" COLLATE utf8_bin, `name` VARCHAR(128) UNIQUE KEY COMMENT "user name" COLLATE utf8_bin) ENGINE = MEMORY');
      })
      ...
      it.skip('should support create table with column_format', () => {
        expect(getParsedSql(`create temporary table if not exists dbname.tableName (id INT(11) auto_increment primary key comment "id column" collate utf8_bin column_format fixed, name varchar(128) unique key comment "user name" collate utf8_bin column_format dynamic) ENGINE = MEMORY`))
          .to.equal('CREATE TEMPORARY TABLE IF NOT EXISTS "dbname"."tableName" ("id" INT(11) AUTOINCREMENT PRIMARY KEY COMMENT "id column" COLLATE utf8_bin COLUMN_FORMAT FIXED, "name" VARCHAR(128) UNIQUE KEY COMMENT "user name" COLLATE utf8_bin COLUMN_FORMAT DYNAMIC) ENGINE = MEMORY');
      })

      it.skip('should support create table with storage', () => {
        expect(getParsedSql(`create temporary table if not exists dbname.tableName (id INT(11) auto_increment primary key comment "id column" collate utf8_bin column_format fixed storage disk, name varchar(128) unique key comment "user name" collate utf8_bin column_format dynamic storage memory) ENGINE = MEMORY`))
          .to.equal('CREATE TEMPORARY TABLE IF NOT EXISTS "dbname"."tableName" ("id" INT(11) AUTOINCREMENT PRIMARY KEY COMMENT "id column" COLLATE utf8_bin COLUMN_FORMAT FIXED STORAGE DISK, "name" VARCHAR(128) UNIQUE KEY COMMENT "user name" COLLATE utf8_bin COLUMN_FORMAT DYNAMIC STORAGE MEMORY) ENGINE = MEMORY');
      })

      it.skip('should support create table with reference definition', () => {
        expect(getParsedSql(`create temporary table if not exists dbname.tableName (id INT(11) auto_increment primary key comment "id column" collate utf8_bin column_format fixed storage disk references rdb.rta(id) match full on delete cascade on update restrict, name varchar(128) unique key comment "user name" collate utf8_bin column_format dynamic storage memory references rdb.rtb(name) match simple on delete set null on update set default) ENGINE = MEMORY`))
          .to.equal('CREATE TEMPORARY TABLE IF NOT EXISTS "dbname"."tableName" ("id" INT(11) AUTOINCREMENT PRIMARY KEY COMMENT "id column" COLLATE utf8_bin COLUMN_FORMAT FIXED STORAGE DISK REFERENCES "rdb"."rta" ("id") MATCH FULL ON DELETE CASCADE ON UPDATE RESTRICT, "name" VARCHAR(128) UNIQUE KEY COMMENT "user name" COLLATE utf8_bin COLUMN_FORMAT DYNAMIC STORAGE MEMORY REFERENCES "rdb"."rtb" ("name") MATCH SIMPLE ON DELETE SET NULL ON UPDATE SET DEFAULT) ENGINE = MEMORY');
      })
      ...
      it.skip('should support binary and columns with or without length', () => {
      ...
      it.skip('should support generated columns with generated always', () =>{
    ...
    describe('create index or key', () => {
      ...
      ['index', 'key'].forEach(type => {
        it.skip(`should support create table ${type}`, () => {
      ...
      it.skip('should support create index multiple columns in tsql', () => {
    ...
    describe('create constraint', () => {
      ...
      it.skip(`should support unique key`, () => {
        ['index', 'key'].forEach(kind => {
          ...
          .to.equal(`CREATE TEMPORARY TABLE "dbname"."tableName" ("id" INT, "name" VARCHAR(128), ${type.toUpperCase()} "idx_name" UNIQUE ${kind.toUpperCase()} USING BTREE ("name") KEY_BLOCK_SIZE 128)`);
        })
      })
      ...
      it(`should support foreign key`, () => {
        expect(getParsedSql(`create temporary table dbname.tableName (id int, name varchar(128), ${type} "idx_name" foreign key index_name (name) references rdb.rta (name_alias) match simple on delete cascade on update set default)`))
          .to.equal(`CREATE TEMPORARY TABLE "dbname"."tableName" ("id" INT, "name" VARCHAR(128), ${type.toUpperCase()} "idx_name" FOREIGN KEY ("name") REFERENCES "rdb"."rta" ("name_alias") MATCH SIMPLE ON DELETE CASCADE ON UPDATE SET DEFAULT)`);
      })
    ...
    describe('create table from like', () => {
      it.skip('should support create table', () => {
    ...
    describe('create table from query', () => {
      it.skip('should support create table from simple select', () => {
      ...
      it.skip('should support create table from union', () => {
    ...
    describe('create trigger pg', () => {
      it.skip('should support basic trigger', () => {
  ...
  describe('create view', () => {
    ...
    it.skip('should support optional setting', () => {
